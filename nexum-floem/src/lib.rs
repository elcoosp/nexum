...
fn register_windows(config: &Config) {
    ...
    for scheme in &config.schemes {
        ...
        key.set_string("", &format!("URL:{} Protocol", scheme)).unwrap();
        key.set_string("URL Protocol", "").unwrap();

        let icon_key = CURRENT_USER.create(&format!("{}\\DefaultIcon", scheme_key)).unwrap();
        let exe_path = std::env::current_exe().unwrap();
        icon_key.set_string("", &format!("{},0", exe_path.to_string_lossy())).unwrap();

        let cmd_key = CURRENT_USER.create(&format!("{}\\shell\\open\\command", scheme_key)).unwrap();
        cmd_key.set_string("", &format!("\"{}\" \"%1\"", exe_path.to_string_lossy())).unwrap();
    }
}
...
fn register_linux(config: &Config) {
    ...
    let app_name = std::env::args().next().unwrap_or_else(|| "nexum-app".to_string());
    ...
    let desktop_entry = format!(
        ...
        app_name,
        exe_str,
        config.schemes.iter().map(|s| format!("x-scheme-handler/{}", s)).collect::<Vec<_>>().join(";")
    );
...
/// Convenience function that creates a Floem RwSignal updated automatically.
#[cfg(feature = "signal")]
